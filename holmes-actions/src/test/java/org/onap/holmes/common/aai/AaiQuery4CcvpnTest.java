...
public class AaiQuery4CcvpnTest {
    ...
    public void test_getLogicLink() {
        ...
        EasyMock.expect(client.get(anyString())).andReturn(data.get("logic-link").toString());
    ...
    public void test_getServiceInstance() {
        EasyMock.expect(client.path(anyString())).andReturn(client);
        EasyMock.expect(client.headers(anyObject())).andReturn(client);
        EasyMock.expect(client.get(anyString())).andReturn(data.get("vpn-binding").toString());

        EasyMock.expect(client.path(anyString())).andReturn(client);
        EasyMock.expect(client.headers(anyObject())).andReturn(client);
        EasyMock.expect(client.get(anyString())).andReturn(data.get("connectivity").toString());

        EasyMock.expect(client.path(anyString())).andReturn(client);
        EasyMock.expect(client.headers(anyObject())).andReturn(client);
        EasyMock.expect(client.get(anyString())).andReturn(data.get("service-instance-by-connectivity").toString());

        EasyMock.expect(client.path(anyString())).andReturn(client);
        EasyMock.expect(client.headers(anyObject())).andReturn(client);
        EasyMock.expect(client.get(anyString())).andReturn(data.get("service-instances-by-service-type").toString());
    ...
    public void test_updateTerminalPointStatus() throws CorrelationException {
        EasyMock.expect(client.path(anyString())).andReturn(client);
        EasyMock.expect(client.headers(anyObject())).andReturn(client);
        EasyMock.expect(client.get(anyString())).andReturn(data.toString());
